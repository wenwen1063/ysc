...
//            pp($search);
            ...
            if ($search['after_type'] != null) {
                $map['ad.after_type']  = $search['after_type'];
            }
            $map['status'] = 1;  //1为以启用

            $seller_id = cookie("seller_id");
            if ($seller_id != 0 || !empty($seller_id)) {
                $map['a.id'] =  array('eq', $seller_id);
                $data          = M()
                    ...
                    ->where($map)
                    ->field('a.*,ad.after_type,s.forshort,s.number as s_number')
                    ->select();
                $count = M()
                    ->table('__ACTIVITY__ a')
                    ->join('left join __ACTIVITY_DISCOUNT__ ad on ad.activity_id = a.id')
            ...
            $seller_id = cookie("seller_id");
            if ($seller_id != 0 || !empty($seller_id)) {
                $cooksj['a.seller_id'] =  array('eq', $seller_id);
                $data = M()
                    ->table('__ACTIVITY__ a')
                    ->join('left join __ACTIVITY_DISCOUNT__ ad on ad.activity_id = a.id')
                    ->join('left join __SELLER__ s on s.id = a.seller_id')
