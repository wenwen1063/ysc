...
class IndexController extends Controller
{
    ...
    private function checkRight($num)
    {
        ...
        if ($id == null) {
            ...
            //拥有权限
            $rabc = M()
                ->table('__ROLEADMIN__ ra')
                ->join("left join __ROLERABC__ rr on rr.role_id = ra.role_id")
                ->where($where = array('ra.admin_id' => $id,'rr.rabc_id' => $num))
                ->field('rr.rabc_id')
                ->find();
            //判断是否拥有权限
...
// 数组去重
    ...
    public function welcome()
    {
        ...
        $end                                 = strtotime(date('Y-m-d')) + 86400; //今日截止
//        $end                                 = time(); //现在时间
