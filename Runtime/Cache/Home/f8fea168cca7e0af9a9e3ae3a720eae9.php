...
<body>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 财务管理 <span class="c-gray en">&gt;</span> 充值提现 <span class="c-gray en">&gt;</span> 商家提现管理
        <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="pd-20">
        <form action="<?php echo U('/home/rechargecash/rechargeseller/search/');?>" method="post/get" name="mainform" class="form form-horizontal" id="form-admin-add" style="float:right;">
            <div class="text-c">
                <input type="text" id="username" name="username" value="<?php echo ($search['username']); ?>" class="input-text" style="width:250px" placeholder="商家名称">
                <input placeholder="申请时间区间" class="laydate-icon" style="height:29px;" value="<?php echo ($search['time']); ?>" name="time" id="time" onClick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
                <input placeholder="申请时间区间" class="laydate-icon" style="height:29px;" value="<?php echo ($search['time2']); ?>" name="time2" id="time2" onClick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
            ...
            <table class="table table-border table-bordered table-bg">
                ...
                <tbody>
                    <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr class="text-c">
                            <td>
                                <?php if ($v['is_ok']==0): ?>
                                <a class="btn btn-primary size-MINI radius" href="<?php echo U('/home/rechargecash/cashpass',array('id'=>$v['id'],'s_id'=>$v['s_id'],'r_money'=>$v['r_money']));?>" onclick="return confirm('确认要通过提现吗？');">通过</a>
                                <a class="btn btn-primary size-MINI radius" href="<?php echo U('/home/rechargecash/cashclose',array('id'=>$v['id'],'s_id'=>$v['s_id'],'r_money'=>$v['r_money']));?>" onclick="return confirm('确认要拒绝提现吗？');">拒绝</a>
                                <?php endif ?>
                            </td>
                            ...
                            <?php if ($v['type']==1): ?>
                                <td>提现</td>
                            <?php endif ?>
                            <td><?php echo ($v['bankclass']); ?></td>
                            <td><?php echo ($v['bankcode']); ?></td>
                            <td><?php echo ($v['bankaccount']); ?></td>
